          ...
          <div className="space-y-3">
            {gaps.map((gap, i) => (
              ...
              >
                ...
                {expandedGap === i && (
                  <div className="px-5 pb-5 pt-0 border-t border-border bg-muted/30 space-y-3">

                    {/* 1. What was written */}
                    {gap.whatWasWritten && (
                      <div className="pt-3">
                        <div className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: 'var(--color-navy)' }}>What was written</div>
                        <p className="text-sm leading-relaxed text-muted-foreground italic">{gap.whatWasWritten}</p>
                      </div>
                    )}

                    {/* 2. What's missing */}
                    {(gap.whatsMissing || gap.issue) && (
                      <div>
                        <div className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: 'var(--color-navy)' }}>What's missing</div>
                        <p className="text-sm leading-relaxed text-foreground">{gap.whatsMissing || gap.issue}</p>
                      </div>
                    )}

                    {/* 3. What to add */}
                    {(gap.whatToAdd || gap.guidance) && (
                      <div className="p-3 rounded-lg border border-border bg-white">
                        <div className="text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: 'var(--color-navy)' }}>What to add</div>
                        <p className="text-sm leading-relaxed">{gap.whatToAdd || gap.guidance}</p>
                      </div>
                    )}
                    ...
                    {gap.example && (
                      <div className="rounded-lg border-2 bg-white overflow-hidden" style={{ borderColor: 'var(--color-gold)' }}>
                        <div className="px-3 py-2 flex items-center justify-between" style={{ background: 'var(--color-gold)' }}>
                          <div className="text-xs font-bold text-white uppercase tracking-wide">Here is a draft — Copy, Edit, and Make It Yours</div>
                          <button
                    ...
                    {gap.helpfulContext && (
                      <div className="p-3 rounded-lg bg-blue-50 border border-blue-100">
                        <div className="text-xs font-semibold uppercase tracking-wide mb-1 text-blue-700">Helpful Context</div>
                        <p className="text-xs leading-relaxed text-blue-800">{gap.helpfulContext}</p>
                      </div>
                    )}
